...
def focal_loss2d(input, target, conf=None, alpha=None ,sim=None , gamma=None, size_average=False, loss_type=None,soft_dev=None,ignore_index=0):
   ...
   #confidence values
    ...
    if conf is not None:
        qt = torch.cat((torch.cuda.FloatTensor(N,c,w,h).zero_(), conf.squeeze().permute(0,3,1,2)), 1)
    else:
